...
class tx_mmdamfilelist_pi1 extends mmlib_extfrontend {
	...
	function main($content,$conf)
		{
		global $xajax;
		$numberfields = array("filterid","showuid","pointer");
		...
		foreach($this->piVars as $key => $value) {
			//t3lib_div::debug($key . '-' . $value);
				if(in_array($key,$numberfields)) {
					...
					$this->piVars[$key] = $GLOBALS['TYPO3_DB']->quoteStr($value);
				}
		    }
	 ...
	 */
 	...
 	function _renderPath($linkText,$folder,$countFolders) {
		...
		//debug("------------------",1);
 		...
 		$link		= $this->pi_linkTP_keepPIvars($linkText,array('getSubFolders' => $folder),$this->allowCaching);
 		 t3lib_div::debug($link,'link');
...
*/
	...
	function getFLEXConversionInfo()
		...
		return array(
			...
			'greybox.' => array (
				//'script' 			=> 'sVIDEO:script',
				'video.' => array (
					'width' 			=> 'sVIDEO:video_width',
					'height' 			=> 'sVIDEO:video_height',
					'title' 			=> 'sVIDEO:video_title',
					),
				'box.'	=> array(
					'width' 			=> 'sVIDEO:box_width',
					'height' 			=> 'sVIDEO:box_height',
					),
				),
	...
	function execQuery($fCountRecords = 0,$strWhereStatement = '')
		...
		$aCateory					= $this->piVars['mode'] && strstr($this->piVars['mode'],'category:') != false ? explode(':',$GLOBALS['TYPO3_DB']->quoteStr($this->piVars['mode'])) : null; // $aCateory[0] = "category", $aCateory[1] = 1
		...
		*/
		...
		if(isset($this->piVars['filterfield']) && isset($this->piVars['filterid'])) {
			...
			$WHERE['filterfield']	= $GLOBALS['TYPO3_DB']->quoteStr($this->piVars['filterfield']) . ' = ' . t3lib_div::intval_positive($this->piVars['filterid']);
		}
	...
	function getFieldContent($fieldname)	{
		...
		switch($fieldname) {
			...
			if(is_readable($this->internal['currentRow']['file_path'] . $filename)) {
			...
			$additionalData['tstamp'] 			= time();
						}

			$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_mmdamfilelist_additionalinfo',$additionalData);
...
//				$GLOBALS["TSFE"]->fe_user->setKey("ses","zipdata_" . $additionalData['uniqueid'],$additionalData);
			...
			case 'preview':
				$filename 	= $this->getUnmodifiedFieldContent('file_name');
				$altprev 	= $this->getUnmodifiedFieldContent('tx_mmdamfilelist_altpreview');
				$hash		= $this->getUnmodifiedFieldContent('tx_mmdamfilelist_oldfilehash');

				// Create new Preview
				/*
				t3lib_div::debug($this->internal['currentRow'],"currow");
				t3lib_div::debug($filename,'filename');
				t3lib_div::debug($this->isImage($filename),'isimage');
				t3lib_div::debug(preg_match('#^altprev_.?#',$altprev),'match');
				t3lib_div::debug($hash,'hash');
				t3lib_div::debug($this->getFileDataHash(),'gfdh');
				*/
				if($this->isImage($filename) &&
					($hash == '' || $hash != $this->getFileDataHash()) &&
					($altprev == '' || preg_match('#^altprev_.?#',$altprev))) {

					$altprev = $this->generateAltPreview();
					//t3lib_div::debug($altprev,'generated image');
				}

				// Use Preview image
				if(strlen($altprev) > 0) {
					//t3lib_div::debug($this->conf['typodbfield.'][$fieldname . '.']['field'],'tfield');
					unset($this->conf['typodbfield.'][$fieldname . '.']['field']);
					$this->conf['typodbfield.'][$fieldname . '.']['path'] = $this->_getUploadFolder();

					$result = $this->getAutoFieldContent($fieldname,$altprev);
					//t3lib_div::debug($altprev,'generated image');
				}
				// Fallback to old version
				else $result = $this->getAutoFieldContent($fieldname,$this->getUnmodifiedFieldContent($this->conf['typodbfield.'][$fieldname . '.']['field']));
			...
			case 'greybox':
				$vconf = $this->conf['greybox.'];
				$subtemplate = $this->_getColContents(0,false,'###GREYBOXTEMPLATE###',array('greybox'),1,true);
				$markerArray['###SCRIPT###'] = $vconf['script'];

				$markerArray['###VIDEO_WIDTH###'] 	= $vconf['video.']['width'];
				$markerArray['###VIDEO_HEIGHT###'] 	= $vconf['video.']['height'];
				$markerArray['###VIDEO_TITLE###'] 	= $vconf['video.']['title'];
				$markerArray['###BOX_WIDTH###'] 	= $vconf['box.']['width'];
				$markerArray['###BOX_HEIGT###'] 	= $vconf['box.']['height'];

				$result = $this->cObj->substituteMarkerArray($subtemplate,$markerArray);
				break;
			//case 'addressfilter':
	...
	*/
	function getAddressPart($fieldname) {
		...
		if($pid == -1) {
			// Rohloff is using both
			if($this->extConf['useinmixedmode'] == true) return;
	...
	*/
	...
	function removeOldDBEntries() {
			$oneWeekOld = time() - (7 * 24 * 60 * 60);
			//$oneWeekOld = time() - (600); // One minute

			$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_mmdamfilelist_additionalinfo','tstamp<"' . $oneWeekOld . '"');
	}

	function generateAltPreview() {
		$fieldname 		= 'preview';
		$path 			= $this->getUnmodifiedFieldContent('file_path');
		$filename 		= $this->getUnmodifiedFieldContent('file_name');
		$graphic 		= t3lib_div::makeInstance('t3lib_stdGraphic');
		$confDBField 	= $this->conf['typodbfield.'][$fieldname . '.'];
		$width			= $confDBField['file.']['width'];	// Base - the preview image
		$height			= $confDBField['file.']['height'];
		$original		= $path . $filename;

		// Base - the Zoom image, image will be resized for the thumbnail
		if($confDBField['imageLinkWrap'] == '1' &&
			count($confDBField['imageLinkWrap.']) > 0 &&
			isset($confDBField['imageLinkWrap.']['width']) &&
			isset($confDBField['imageLinkWrap.']['height'])) {

			$width			= $confDBField['imageLinkWrap.']['width'];	// Base - the preview image
			$height			= $confDBField['imageLinkWrap.']['height'];
		}

		$graphic->init();
		$image = $graphic->imageMagickConvert($path . $filename,'png',
			$width,$height,
			'','','', 1);
		/*
		t3lib_div::debug($filename, '$filename');
		t3lib_div::debug($width,'$width');
		t3lib_div::debug($height,'$height');
		t3lib_div::debug($confDBField,'$widt$confDBField');
		t3lib_div::debug($image);
		*/
		// typo3temp/pics/594d5b7f73.jpg
		if($image) {
			//t3lib_div::debug($image,'image true');
			$aTargetFileName 	= t3lib_div::split_fileref($filename);
			$newFile 			= 'altprev_' . $aTargetFileName['filebody'] . '.' . $image[2]; // 2 - extensions
			$convertedFile 		= $image[3];

			if($convertedFile == $original) die("To rename the original file would destroy the whole thing. Please contace office@bitcon.at");
			//t3lib_div::debug($original,'$original');

			$convertedFileSplit = t3lib_div::split_fileref($convertedFile);
			$convertedFileNameOnly = $convertedFileSplit['filebody'] . '.' . $convertedFileSplit['fileext'];

			if(!file_exists($this->_getUploadFolder() . $convertedFileNameOnly)){
				if(mmlib_FileHandling::renameFile($convertedFile,$this->_getUploadFolder() . $convertedFileNameOnly)) {

					// Write to DB
					$this->updateAltPreview($this->getUnmodifiedFieldContent('uid'),$convertedFileNameOnly);
					//t3lib_div::debug($newFile,'newfile -  OK');
				}
			}
		}

	$altPreview = $this->getUnmodifiedFieldContent('tx_mmdamfilelist_altpreview');


	return (is_file(PATH_site . $this->_getUploadFolder() . $altPreview) ? $altPreview : false);
	}

	function updateAltPreview($uid,$filename) {
		$hash = $this->getFileDataHash();
		$updateArray = array(
			'tx_mmdamfilelist_altpreview' => $filename,
			'tx_mmdamfilelist_oldfilehash' => $hash,
			);
		...
		$res = $GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_dam', "uid='" . $uid . "'", $updateArray);
		//t3lib_div::debug($res,"res - update");
		if($res) {
			// Update the current RECORD internaly
			$this->internal['currentRow']['tx_mmdamfilelist_altpreview'] = $filename;
			$this->internal['currentRow']['tx_mmdamfilelist_oldfilehash'] = $hash;
		}

		if($showLastQuery && $res == null) {
			t3lib_div::debug($GLOBALS['TYPO3_DB']->debug_lastBuiltQuery,"lastBuiltQuery=");
			}
	}

	function getFileDataHash() {
		$md5Base = $this->getUnmodifiedFieldContent('file_name');
		$md5Base += $this->getUnmodifiedFieldContent('file_path');
		$md5Base += $this->getUnmodifiedFieldContent('file_size');
		$md5Base += $this->getUnmodifiedFieldContent('file_mtime');
		$md5Base += $this->getUnmodifiedFieldContent('file_ctime');

		$md5Base += $this->conf['typodbfield.']['preview' . '.']['file.']['width'];
		$md5Base += $this->conf['typodbfield.']['preview' . '.']['file.']['height'];

		$md5Base += $this->conf['typodbfield.']['preview' . '.']['imageLinkWrap.']['width'];
		$md5Base += $this->conf['typodbfield.']['preview' . '.']['imageLinkWrap.']['height'];

		return t3lib_div::shortmd5($md5Base);
	}
}
